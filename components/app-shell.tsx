...
import { Calendar, FileText, ClipboardList, Menu, Moon, Sun, LogOut } from "lucide-react";
...
const managerNavItems = [
  { href: "/submit", label: "Petición", icon: FileText },
  { href: "/my-requests", label: "Mis Peticiones", icon: ClipboardList },
];
    ...
    <div className="flex h-full flex-col bg-sidebar text-sidebar-foreground">
      <div className="flex h-16 items-center gap-3 border-b border-sidebar-border px-4">
        ...
        <div className="flex flex-col">
          <span className="text-sm font-semibold leading-tight">Bienestar</span>
      ...
      {user && (
        <div className="border-t border-sidebar-border p-3 space-y-1">
          ...
          <p className="px-3 text-xs text-sidebar-foreground/40 truncate capitalize">{user.role}</p>
        </div>
      )}
...
function Header() {
  ...
  return (
    <header className="flex h-16 items-center gap-4 border-b bg-card px-4 md:px-6">
      <MobileNav />
      <div className="flex h-8 w-8 items-center justify-center rounded bg-sidebar md:hidden">
        <span className="text-sm font-bold text-white">UV</span>
      </div>
